...
router.get('/pay/:id', (req, res) => {
	Bookevnt.find({_id: req.params.id}, function (err, payMreq) {
		if(err) {
			...
			params['CALLBACK_URL']		= 'http://localhost:'+port+'/api/callback';
    ...
    req.on('end', function () {
    	...
    	if(post_data.RESPCODE == '01') {
			Bookevnt.find({ORDER_ID: post_data.ORDERID}, function (err, paySuc) {
				if(err) {
					res.writeHead(200, {'Content-Type': 'text/html'});
					res.write('<script type="text/javascript">window.location.replace("http://localhost:4200/response/false");</script>');
					res.end();
				} else {
					if(!paySuc) {
						res.writeHead(200, {'Content-Type': 'text/html'});
						...
						paySuc[0].save(function(err, updPayment){
							if(err){
								...
								res.writeHead(200, {'Content-Type': 'text/html'});
								res.write('<script type="text/javascript">window.location.replace("http://localhost:4200/response/'+paySuc[0]._id+'");</script>');
								res.end();
							}
						});
					}
